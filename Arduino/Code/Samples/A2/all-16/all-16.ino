...
void setup() {
  ...
  // Load the model
  tflite = new TFLite();
  tflite->load_model("/path/to/RFC_model.tflite", tensorArena, tensorArenaSize);
}
...
// Bluetooth command handling
  ...
  // Prepare input data for the TensorFlow Lite model
  float input_data[] = {mappedSoilMoistureValue, temperatureC}; // Replace with actual inputs

  TfLiteTensor* input = tflite->get_input_tensor(0);
  ...
  // Get the output
  TfLiteTensor* output = tflite->get_output_tensor(0);
  int prediction = (int)output->data.f[0]; // Get the prediction result

  // Use the prediction result to control the pump
  if (prediction == 1) { // Example condition for turning on the pump
    pumpStatus = true;
    digitalWrite(relayPin, HIGH);
  } else {
    pumpStatus = false;
