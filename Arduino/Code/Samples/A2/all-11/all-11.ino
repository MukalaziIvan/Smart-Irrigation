...
#define FIREBASE_AUTH F("YOUR_FIREBASE_AUTH_KEY")
...
const int moistureThresholdMin = 30; // Minimum moisture level for turning the pump on
const int moistureThresholdMax = 60; // Maximum moisture level for turning the pump off
const float temperatureThresholdMin = 20.0; // Minimum temperature for turning the pump on
const float temperatureThresholdMax = 30.0; // Maximum temperature for turning the pump off
...
void loop() {
  ...
  String timestamp = String(currentMillis); // Use millis() as the timestamp
  Firebase.setInt(firebaseData, "/sensor_readings/" + timestamp + "/soil_moisture", mappedSoilMoistureValue);
  Firebase.setInt(firebaseData, "/sensor_readings/" + timestamp + "/ldr", ldrValue);
  Firebase.setFloat(firebaseData, "/sensor_readings/" + timestamp + "/temperature", temperatureC);

  // Check if the conditions for turning on the pump are met
  bool turnOnPump = (mappedSoilMoistureValue < moistureThresholdMin && temperatureC > temperatureThresholdMin && temperatureC < temperatureThresholdMax);
